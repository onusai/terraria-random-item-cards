const itemData = {
    "pre-boss": {
        "mele":[
            ["Gold Broadsword", "Platinum Broadsword"],
            "Mandible Blade",
            "Katana",
            "Ice Blade",
            "Enchanted Sword",
            "Terragrim",
            "Arkhalis",
            "Starfury",
            "Falcon Blade",
            "Light's Bane",
            "Blood Butcherer",
            "Blade of Grass",
            "Rally",
            "Malaise",
            "Artery",
            "Amazon",
            "Spear",
            "Trident",
            "Storm Spear",
            "Swordfish",
            "The Rotted Fork",
            "Enchanted Boomerang",
            "Shroomerang",
            "Ice Boomerang",
            "Trimarang",
            "Thorn Chakram",
            "Flaming Mace",
            "Ball O' Hurt"
        ],
        "range":[
            ["Gold Bow", "Platinum Bow"],
            "Demon Bow",
            "Tendon Bow",
            "Blood Rain Bow",
            "Snowball Cannon",
            "Sandgun",
            "The Undertaker",
            "Musket",
            "Minishark",
            "Revolver",
            "Boomstick",
            "Blowpipe",
            "Poisoned Knife",
            "Grenade"
        ],
        "magic":[
            ["Ruby Staff", "Diamond Staff"],
            "Thunder Zapper",
            "Crimson Rod",
            "Vilethorn",
            "Demon Scythe"
        ],
        "summon":[
            "Leather Whip",
            "Snapthorn",
            "Finch Staff",
            "Abigail's Flower",
            "Slime Staff",
            "Vampire Frog Staff",
            "Flinx Staff"
        ],
        "mixed":[
    
        ],
        "acc":[
            "Magma Stone",
            "White String",
            "Counterweights",
            "Magic Cuffs",
            "Mana Flower",
            "Celestial Magnet",
            "Magnet Flower",
            "Celestial Cuffs",
            "Shark Tooth Necklace",
            "Feral Claws",
            "Hermes Boots",
            "Amphibian Boots",
            "Lightning Boots",
            "Magiluminescence",
            "Cloud in a Bottle",
            "Blizzard in a Bottle",
            "Sandstorm in a Bottle",
            "Tsunami in a Bottle",
            "Shiny Red Balloon",
            [
                "Cloud in a Balloon",
                "Blizzard in a Balloon",
                "Sandstorm in a Balloon"
            ],
            [
                "Blue Horseshoe Balloon",
                "Yellow Horseshoe Balloon",
                "White Horseshoe Balloon"
            ],
            "Bundle of Balloons",
            "Band of Regeneration",
            "Frog Leg",
            "Lucky Horseshoe",
            "Panic Necklace",
            "Shark Tooth Necklace",
            "Fledgling Wings"
        ],
        "armor": {
            "all": [
                "Ancient Shadow armor",
                "Fossil armor",
                "Ancient Cobalt armor",
                "Jungle armor",
                "Pumpkin armor",
                "Gold armor",
                "Platinum armor"
            ],
            "head": [
                "Magic Hat",
                "Wizard Hat"
            ],
            "chest": [
                "Diamond Robe",
                "Mystic Robe",
                "Flinx Fur Coat"
            ],
            "legs": []
        }
    },
    "pre-hm": {
        "mele":[
            "Terragrim",
            "Arkhalis",
            "Enchanted Sword",
            "Starfury",
            "Bee Keeper",
            "Falcon Blade",
            "Muramasa",
            "Volcano",
            "Night's Edge",
            "Dark Lance",
            "Sunfury",
            "Flamarang",
            "Amazon",
            "Valor",
            "Cascade",
            "Code 1",
            "Hive-Five"
        ],
        "range":[
            "Grenade",
            "Beenade",
            "Star Cannon",
            "Minishark",
            "Handgun",
            "Boomstick",
            "Phoenix Blaster",
            "Quad-Barrel Shotgun",
            "Revolver",
            "Blood Rain Bow",
            "Molten Fury",
            "The Bee's Knees",
            "Hellwing Bow"
        ],
        "magic":[
            [
                "Diamond Staff",
                "Ruby Staff",
                "Amber Staff"
            ],
            "Vilethorn",
            "Crimson Rod",
            "Space Gun",
            "Bee Gun",
            "Aqua Scepter",
            "Water Bolt",
            "Flower of Fire",
            "Flamelash",
            "Demon Scythe",
            "Gray Zapinator",
            "Magic Missile"
        ],
        "summon":[
            "Hornet Staff",
            "Vampire Frog Staff",
            "Imp Staff",
            "Snapthorn",
            "Spinal Tap",
            "Ballista Rod",
            "Lightning Aura Rod"
        ],
        "mixed":[
            "Dynamite"
        ],
        "acc":[
            "Counterweights",
            "Strings",
            "Magma Stone",
            "Celestial Cuffs",
            "Mana Flower",
            "Magnet Flower",
            "Feral Claws",
            "Pygmy Necklace",
            "Shield of Cthulhu",
            "Worm Scarf",
            "Brain of Confusion",
            "Fledgling Wings",
            [
                "Cloud in a Balloon",
                "Blizzard in a Balloon",
                "Sandstorm in a Balloon"
            ],
            [
                "Blue Horseshoe Balloon",
                "Yellow Horseshoe Balloon",
                "White Horseshoe Balloon"
            ],
            "Bundle of Balloons",
            "Lightning Boots",
            "Terraspark Boots",
            "Amphibian Boots",
            "Band of Regeneration",
            "Obsidian Shield",
            "Stinger Necklace"
        ],
        "armor": {
            "all": [
                "Shadow armor",
                "Molten armor",
                "Necro armor",
                "Fossil armor",
                "Meteor armor",
                "Jungle armor",
                "Ancient Cobalt armor",
                "Bee armor",
                "Obsidian armor",
                "Pumpkin armor",
                "Ninja armor",
                "Gladiator armor",
                "Platinum armor",
                "Crimson armor"
            ],
            "head": [
                "Wizard Hat"
            ],
            "chest": [
                [
                    "Amber Robe",
                    "Diamond Robe"
                ],
                "Mystic Robe",
                "Flinx Fur Coat",
                "Gi"
            ],
            "legs": [
    
            ]
        }
    },
    "pre-mech": {
        "mele":[
            "Gradient",
            "Amarok",
            "Chik",
            "Chain Guillotines",
            "Hel-Fire",
            "Dao of Pow",
            "Bladetongue",
            "Ice Sickle",
            "Bananarang",
            "Fetid Baghnakhs",
            "Shadowflame Knife",
            "Sergeant United Shield",
            "Drippler Crippler",
            "Flying Knife",
            "Ham Bat",
            "Obsidian Swordfish",
        ],
        "range":[
            "Coin Gun",
            "Adamantite Repeater",
            "Titanium Repeater",
            "Shadowflame Bow",
            "Marrow",
            "Daedalus Stormbow",
            "Clockwork Assault Rifle",
            "Onyx Blaster",
            "Uzi",
            "Dart Rifle",
            "Star Cannon",
            "Dart Pistol",
            "Toxikarp"
        ],
        "magic":[
            "Meteor Staff",
            "Crystal Serpent",
            "Golden Shower",
            "Magic Dagger",
            "Cursed Flames",
            "Crystal Storm",
            "Shadowflame Hex Doll",
            "Sky Fracture",
            "Frost Staff",
            "Spirit Flame",
            "Life Drain",
            "Clinger Staff",
            "Nimbus Rod",
            "Orange Zapinator"
        ],
        "summon":[
            "Spider Staff",
            "Queen Spider Staff",
            "Pirate Staff",
            "Tempest Staff",
            "Sanguine Staff",
            "Blade Staff",
            "Firecracker",
            "Cool Whip"
        ],
        "mixed":[
    
        ],
        "acc":[
            "Warrior Emblem",
            "Yoyo Bag",
            "Berserker's Glove",
            "Magma Stone",
            "Moon Charm",
            "Ranger Emblem",
            "Magic Quiver",
            "Molten Quiver",
            "Stalker's Quiver",
            "Arcane Flower",
            "Celestial Cuffs",
            "Magnet Flower",
            "Mana Cloak",
            "Sorcerer Emblem",
            "Summoner Emblem",
            "Berserker's Glove",
            "Pygmy Necklace",
            "Lightning Boots",
            "Terraspark Boots",
            "Amphibian Boots",
            "Obsidian Shield",
            "Ankh Shield",
            "Charm of Myths",
            "Putrid Scent",
            "Flesh Knuckles",
            ["Fairy Wings",
            "Fin Wings"],
            ["Frozen Wings",
            "Harpy Wings"],
            ["Bee Wings",
            "Butterfly Wings"],
            ["Angel Wings",
            "Demon Wings"],
            ["Leaf Wings",
            "Jetpack"],
            

        ],
        "armor": {
            "all": [
                "Palladium armor",
                "Mythril armor",
                "Adamantite armor",
                "Titanium armor",
                "Frost Armor",
                "Spider armor",
                "Forbidden armor",
                "Obsidian armor",
                "Crystal Assassin armor"
            ],
            "head": [
    
            ],
            "chest": [
                
            ],
            "legs": [
            ]
        }
    },
    "pre-plant": {
        "mele":[
            "True Night's Edge",
            "True Excalibur",
            "Yelets",
            "Amarok",
            "Code 2",
            "Fetid Baghnakhs",
            "Death Sickle",
            "Shadowflame Knife",
            "Light Disc"
        ],
        "range":[
            "Coin Gun",
            "Megashark",
            "Uzi",
            "Daedalus Stormbow",
            "Marrow",
            "Chlorophyte Shotbow",
            "Hallowed Repeater",
            "Dart Rifle",
            "Dart Pistol",
            "Super Star Shooter"
        ],
        "magic":[
            "Meteor Staff",
            "Sky Fracture",
            "Golden Shower",
            "Frost Staff",
            "Crystal Serpent",
            "Rainbow Rod",
            "Magical Harp",
            "Unholy Trident",
            "Magic Dagger",
            "Clinger Staff",
            "Venom Staff"
        ],
        "summon":[
            "Blade Staff",
            "Spider Staff",
            "Optic Staff",
            "Sanguine Staff",
            "Ballista Cane",
            "Lightning Aura Cane",
            "Queen Spider Staff",
            "Firecracker",
            "Durendal",
            "Tempest Staff"
        ],
        "mixed":[
    
        ],
        "acc":[
            "Warrior Emblem",
            "Mechanical Glove",
            "Fire Gauntlet",
            "Moon Charm",
            "Yoyo Bag",
            "Ranger Emblem",
            "Magic Quiver",
            "Molten Quiver",
            "Stalker's Quiver",
            "Celestial Emblem",
            "Celestial Cuffs",
            "Sorcerer Emblem",
            "Magnet Flower",
            "Summoner Emblem",
            "Monk's Belt",
            "Berserker's Glove",
            "Pygmy Necklace",
            "Avenger Emblem",
            "Lightning Boots",
            "Terraspark Boots",
            "Amphibian Boots",
            "Obsidian Shield",
            "Ankh Shield",
            "Charm of Myths",
            "Flame Wings",
            "Bat Wings",
            "Steampunk Wings"
        ],
        "armor": {
            "all": [
                "Turtle armor",
                "Chlorophyte armor",
                "Hallowed armor",
                "Spider armor",
                "Squire armor",
                "Monk armor",
                "Apprentice armor",
                "Huntress armor",
                "Forbidden armor"
            ],
            "head": [
            ],
            "chest": [
            ],
            "legs": [
            ]
        }
    },
    "pre-golem": {
        "mele":[
            "Terra Blade",
            "The Horseman's Blade",
            "Seedler",
            "Flower Pow",
            "Keybrand",
            "The Eye of Cthulhu",
            "Kraken",
            "Shadow Jousting Lance",
            "Vampire Knives",
            "Scourge of the Corruptor",
            "Paladin's Hammer"
        ],
        "range":[
            "Megashark",
            "Piranha Gun",
            "Sniper Rifle",
            "Coin Gun",
            "Tactical Shotgun",
            "Chain Gun",
            "Daedalus Stormbow",
            "Chlorophyte Shotbow",
            "Rocket Launcher",
            "Pulse Bow",
            "Nail Gun",
            "Marrow",
            "Candy Corn Rifle",
            "Stake Launcher",
            "Venus Magnum",
            "Proximity Mine Launcher"
        ],
        "magic":[
            "Bat Scepter",
            "Venom Staff",
            "Nettle Burst",
            "Shadowbeam Staff",
            "Spectre Staff",
            "Inferno Fork",
            "Magnet Sphere",
            "Rainbow Gun",
            "Resonance Scepter",
            "Toxic Flask",
            "Clinger Staff",
            "Razorpine"
        ],
        "summon":[
            "Blade Staff",
            "Raven Staff",
            "Desert Tiger Staff",
            "Sanguine Staff",
            "Pygmy Staff",
            "Terraprisma",
            "Staff of the Frost Hydra",
            "Queen Spider Staff",
            "Morning Star",
            "Dark Harvest",
            "Kaleidoscope"
        ],
        "mixed":[
    
        ],
        "acc":[
            "Warrior Emblem",
            "Mechanical Glove",
            "Fire Gauntlet",
            "Frozen Shield",
            "Yoyo Bag",
            "Moon Charm",
            "Ranger Emblem",
            "Magic Quiver",
            "Molten Quiver",
            "Stalker's Quiver",
            "Celestial Emblem",
            "Celestial Cuffs",
            "Sorcerer Emblem",
            "Magnet Flower",
            "Summoner Emblem",
            "Hercules Beetle",
            "Papyrus Scarab",
            "Necromantic Scroll",
            "Pygmy Necklace",
            "Berserker's Glove",
            "Avenger Emblem",
            "Lightning Boots",
            "Terraspark Boots",
            "Amphibian Boots",
            "Ankh Shield",
            "Charm of Myths",
            ["Spectre Wings", "Empress Wings", "Betsy's Wings"],
            ["Tattered Fairy Wings", "Spooky Wings", "Festive Wings", "Beetle Wings"],
            ["Mothron Wings", "Flame Wings"],
            "Hoverboard",
            "Moon Stone"
    
        ],
        "armor": {
            "all": [
                "Chlorophyte armor",
                "Shroomite armor",
                "Spectre armor",
                "Spooky armor",
                "Tiki armor",
                "Hallowed armor",
                "Squire armor",
                "Turtle armor",
                "Monk armor",
                "Apprentice armor"
            ],
            "head": [
            ],
            "chest": [
    
            ],
            "legs": [
                "Mythril Greaves"
            ]
        }
    },
    "pre-lunar": {
        "mele":[
            "Terra Blade",
            "Flying Dragon",
            "Seedler",
            "The Horseman's Blade",
            "North Pole",
            "Influx Waver",
            "Flairon",
            "The Eye of Cthulhu",
            "Vampire Knives",
            "Possessed Hatchet",
            "Starlight",
            "Golem Fist",
            "Daybreak"
        ],
        "range":[
            "Xenopopper",
            "Coin Gun",
            "Sniper Rifle",
            "Chain Gun",
            "Electrosphere Launcher",
            "Snowman Cannon",
            "Tsunami",
            "Eventide",
            "Aerial Bane",
            "Stynger",
            "Phantasm",
            "Vortex Beater"
        ],
        "magic":[
            "Razorpine",
            "Blizzard Staff",
            "Razorblade Typhoon",
            "Laser Machinegun",
            "Charged Blaster Cannon",
            "Bubble Gun",
            "Magnet Sphere",
            "Nightglow",
            "Betsy's Wrath",
            "Stellar Tune",
            "Staff of Earth",
            "Heat Ray",
            "Nebula Arcanum"
        ],
        "summon":[
            "Xeno Staff",
            "Raven Staff",
            "Ballista Staff",
            "Lightning Aura Staff",
            "Terraprisma",
            "Morning Star",
            "Dark Harvest",
            "Kaleidoscope",
            "Stardust Cell Staff",
            "Stardust Dragon Staff"
        ],
        "mixed":[
    
        ],
        "acc":[
            "Warrior Emblem",
            "Mechanical Glove",
            "Fire Gauntlet",
            "Frozen Shield",
            "Yoyo Bag",
            "Avenger Emblem",
            "Destroyer Emblem",
            "Ranger Emblem",
            "Magic Quiver",
            "Sniper Scope",
            "Celestial Emblem",
            "Celestial Cuffs",
            "Sorcerer Emblem",
            "Pygmy Necklace",
            "Summoner Emblem",
            "Hercules Beetle",
            "Necromantic Scroll",
            "Papyrus Scarab",
            "Destroyer Emblem",
            "Avenger Emblem",
            "Celestial Stone",
            "Celestial Shell",
            "Amphibian Boots",
            "Terraspark Boots",
            "Lightning Boots",
            "Ankh Shield",
            "Charm of Myths",
            "Fishron Wings",
            "Hoverboard"
        ],
        "armor": {
            "all": [
                "Beetle armor",
                "Hallowed armor",
                "Chlorophyte armor",
                "Shinobi Infiltrator armor",
                "Valhalla Knight armor",
                "Monk armor",
                "Shroomite armor",
                "Red Riding armor",
                "Spectre armor",
                "Dark Artist armor",
                "Spooky armor",
                "Tiki armor",
                "Squire armor"
            ],
            "head": [
    
            ],
            "chest": [
                
            ],
            "legs": [
    
            ]
        }
    },
    "post-ml": {
        "mele":[
            "Zenith",
            "Meowmere",
            "Star Wrath",
            "Solar Eruption",
            "Daybreak",
            "Terrarian",
            "Vampire Knives",
            "Flying Dragon"
        ],
        "range":[
            "S.D.M.G.",
            "Vortex Beater",
            "Phantasm",
            "Celebration Mk2",
            "Snowman Cannon"
        ],
        "magic":[
            "Last Prism",
            "Lunar Flare",
            "Nebula Arcanum",
            "Nebula Blaze",
            "Magnet Sphere",
            "Razorblade Typhoon"
        ],
        "summon":[
            "Stardust Dragon Staff",
            "Stardust Cell Staff",
            "Lunar Portal Staff",
            "Rainbow Crystal Staff",
            "Ballista Staff",
            "Terraprisma",
            "Kaleidoscope"
        ],
        "mixed":[
    
        ],
        "acc":[
            "Warrior Emblem",
            "Mechanical Glove",
            "Fire Gauntlet",
            "Frozen Shield",
            "Yoyo Bag",
            "Ranger Emblem",
            "Molten Quiver",
            "Stalker's Quiver",
            "Magic Quiver",
            "Recon Scope",
            "Sniper Scope",
            "Celestial Emblem",
            "Celestial Cuffs",
            "Sorcerer Emblem",
            "Mana Regeneration Band",
            "Arcane Flower",
            "Mana Cloak",
            "Papyrus Scarab",
            "Summoner Emblem",
            "Hercules Beetle",
            "Necromantic Scroll",
            "Pygmy Necklace",
            "Ankh Shield",
            "Avenger Emblem",
            "Celestial Shell",
            "Celestial Stone",
            "Destroyer Emblem",
            "Amphibian Boots",
            "Terraspark Boots",
            "Charm of Myths",
            "Solar Wings",
            "Stardust Wings",
            "Nebula Mantle",
            "Vortex Booster",
            "Betsy's Wings",
            "Celestial Starboard",
            "Master Ninja Gear"
        ],
        "armor": {
            "all": [
                "Solar Flare armor",
                "Shinobi Infiltrator armor",
                "Monk armor",
                "Vortex armor",
                "Nebula armor",
                "Stardust armor",
                "Valhalla Knight armor",
                "Squire armor"
            ],
            "head": [
    
            ],
            "chest": [
                
            ],
            "legs": [
            ]
        }
    }
}

const itemExpands = {
    "Ancient Shadow armor":[
        "Ancient Shadow Helmet",
        "Ancient Shadow Scalemail",
        "Ancient Shadow Greaves"
    ],
    "Fossil armor":[
        "Fossil Helmet",
        "Fossil Plate",
        "Fossil Greaves"
    ],
    "Necro armor":[
        "Necro Helmet",
        "Necro Breastplate",
        "Necro Greaves"
    ],
    "Ancient Cobalt armor":[
        "Ancient Cobalt Helmet",
        "Ancient Cobalt Breastplate",
        "Ancient Cobalt Leggings"
    ],
    "Jungle armor":[
        "Jungle Hat",
        "Jungle Shirt",
        "Jungle Pants"
    ],
    "Pumpkin armor":[
        "Pumpkin Helmet",
        "Pumpkin Breastplate",
        "Pumpkin Leggings"
    ],
    "Gold armor":[
        "Gold Helmet",
        "Gold Chainmail",
        "Gold Greaves"
    ],
    "Platinum armor":[
        "Platinum Helmet",
        "Platinum Chainmail",
        "Platinum Greaves"
    ],
    "Shadow armor":[
        "Shadow Helmet",
        "Shadow Scalemail",
        "Shadow Greaves"
    ],
    "Molten armor":[
        "Molten Helmet",
        "Molten Breastplate",
        "Molten Greaves"
    ],
    "Meteor armor":[
        "Meteor Helmet",
        "Meteor Suit",
        "Meteor Leggings"
    ],
    "Bee armor":[
        "Bee Headgear",
        "Bee Breastplate",
        "Bee Greaves"
    ],
    "Obsidian armor":[
        "Obsidian Outlaw Hat",
        "Obsidian Longcoat",
        "Obsidian Pants"
    ],
    "Ninja armor":[
        "Ninja Hood",
        "Ninja Shirt",
        "Ninja Pants"
    ],
    "Gladiator armor":[
        "Gladiator Helmet",
        "Gladiator Breastplate",
        "Gladiator Leggings"
    ],
    "Crimson armor":[
        "Crimson Helmet",
        "Crimson Scalemail",
        "Crimson Greaves"
    ],
    "Palladium armor":[
        ["Palladium Headgear", "Palladium Mask", "Palladium Helmet"],
        "Palladium Breastplate",
        "Palladium Leggings"
    ],
    "Mythril armor":[
        ["Mythril Hood", "Mythril Helmet", "Mythril Hat"],
        "Mythril Chainmail",
        "Mythril Greaves"
    ],
    "Adamantite armor":[
        ["Adamantite Headgear", "Adamantite Helmet", "Adamantite Mask"],
        "Adamantite Breastplate",
        "Adamantite Leggings"
    ],
    "Titanium armor":[
        ["Titanium Headgear", "Titanium Mask", "Titanium Helmet"],
        "Titanium Breastplate",
        "Titanium Leggings"
    ],
    "Frost Armor":[
        "Frost Helmet",
        "Frost Breastplate",
        "Frost Leggings"
    ],
    "Spider armor":[
        "Spider Mask",
        "Spider Breastplate",
        "Spider Greaves"
    ],
    "Forbidden armor":[
        "Forbidden Mask",
        "Forbidden Robes",
        "Forbidden Treads"
    ],
    "Crystal Assassin armor":[
        "Crystal Assassin Hood",
        "Crystal Assassin Shirt",
        "Crystal Assassin Pants"
    ],
    "Turtle armor":[
        "Turtle Helmet",
        "Turtle Scale Mail",
        "Turtle Leggings"
    ],
    "Chlorophyte armor":[
        ["Chlorophyte Mask", "Chlorophyte Helmet", "Chlorophyte Headgear"],
        "Chlorophyte Plate Mail",
        "Chlorophyte Greaves"
    ],
    "Hallowed armor":[
        ["Hallowed Mask", "Hallowed Helmet", "Hallowed Headgear", "Hallowed Hood"],
        "Hallowed Plate Mail",
        "Hallowed Greaves"
    ],
    "Squire armor":[
        "Squire's Great Helm",
        "Squire's Plating",
        "Squire's Greaves"
    ],
    "Monk armor":[
        "Monk's Bushy Brow Bald Cap",
        "Monk's Shirt",
        "Monk's Pants"
    ],
    "Apprentice armor":[
        "Apprentice's Hat",
        "Apprentice's Robe",
        "Apprentice's Trousers"
    ],
    "Huntress armor":[
        "Huntress's Wig",
        "Huntress's Jerkin",
        "Huntress's Pants"
    ],
    "Shroomite armor":[
        ["Shroomite Headgear", "Shroomite Helmet", "Shroomite Mask"],
        "Shroomite Breastplate",
        "Shroomite Leggings"
    ],
    "Spectre armor":[
        ["Spectre Mask", "Spectre Hood"],
        "Spectre Robe",
        "Spectre Pants"
    ],
    "Spooky armor":[
        "Spooky Helmet",
        "Spooky Breastplate",
        "Spooky Leggings"
    ],
    "Tiki armor":[
        "Tiki Mask",
        "Tiki Shirt",
        "Tiki Pants"
    ],
    "Beetle armor":[
        "Beetle Helmet",
        ["Beetle Scale Mail", "Beetle Shell"],
        "Beetle Leggings"
    ],
    "Shinobi Infiltrator armor":[
        "Shinobi Infiltrator's Helmet",
        "Shinobi Infiltrator's Torso",
        "Shinobi Infiltrator's Pants"
    ],
    "Valhalla Knight armor":[
        "Valhalla Knight's Helm",
        "Valhalla Knight's Breastplate",
        "Valhalla Knight's Greaves"
    ],
    "Red Riding armor":[
        "Red Riding Hood",
        "Red Riding Dress",
        "Red Riding Leggings"
    ],
    "Dark Artist armor":[
        "Dark Artist's Hat",
        "Dark Artist's Robes",
        "Dark Artist's Leggings"
    ],
    "Solar Flare armor":[
        "Solar Flare Helmet",
        "Solar Flare Breastplate",
        "Solar Flare Leggings"
    ],
    "Vortex armor":[
        "Vortex Helmet",
        "Vortex Breastplate",
        "Vortex Leggings"
    ],
    "Nebula armor":[
        "Nebula Helmet",
        "Nebula Breastplate",
        "Nebula Leggings"
    ],
    "Stardust armor":[
        "Stardust Helmet",
        "Stardust Plate",
        "Stardust Leggings"
    ]
}